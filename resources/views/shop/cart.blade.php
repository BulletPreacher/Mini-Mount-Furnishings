...
@section('content')
    ...
    <table id="cart" class="table table-hover table-condensed">
        ...
        @if(session('cart'))
            @foreach(session('cart') as $id => $details)
                ...
                <tr>
                    <td data-th="Product">
                        <div class="row">
                            <div class="col-sm-3 hidden-xs"><img src="{{ $details['image'] }}" style="width: 100px"
        ...
        <tfoot>
        <tr>
            <td><a href="{{ url('/shop/index') }}" class="btn btn-warning" style="background-color: white"><i class="fa fa-angle-left"></i> Continue Shopping</a>

            </td>
            <td colspan="2" class="hidden-xs"></td>
            <td class="hidden-xs text-center"><strong>Total R{{ $total }}</strong></td>
            <td>
                <a href="{{ url('/checkout') }}" class="btn btn-outline-danger">Checkout</a>
            </td>
        </tr>
        </tfoot>
    </table>
